...
[DualHolo Analyze v0.4.0の配布](https://github.com/dainakai/ArcImgAcquisition/releases/tag/analyzer-v0.4.0)から対象OSのアーカイブを展開し、`DualHoloAnalyze`を起動してください。
...
3. **Analyze · 深度探索**で計算します。キャリブレーション適用後のcam0 Gabor・位相回復では、最初に指定回数のGSを一度実行して両方の再生に備えます。Tamura曲線はここで選んだモードで計算します。
4. 完了すると左側が「深度・保存」に切り替わります。スライダー、Tamura曲線のクリック、ピーク候補の選択で、その深度を平均値パディング（既定4k）で再生します。任意の深度は数値入力してEnterで再生します。数値欄の下に独立した「上下の刻み」行を設け、0.1 mm／0.2 mm／1 mm／2 mmから選べます。上下ボタンを押した時点で計算が始まります。
5. 再生像上部の**cam0 Gabor／位相回復**で、同じ深度・拡大率・スクロール位置のまま両像を比較します。適用済みキャリブレーションと両カメラ画像があり、cam0 Gaborまたは位相回復でAnalyzeした後に使えます。未較正・不適合の場合は無効です。
...
cam0 Gabor／位相回復の比較では、cam0面の振幅はどちらも`√I0`とし、Gaborの位相を0、位相回復の位相をGSの結果とします。
各深度で両モード・帯域制限あり／なしの4種類を準備し、表示切替ではFFTやGSを再実行しません。
4種類に共通の明暗範囲を使い、Nでコントラスト正規化を解除した場合も共通の強度範囲を保ちます。
保持する再生像は表示中の深度だけです。深度変更時は両モードを再生し、GS自体は繰り返しません。
左側の「Tamura：…」は曲線を計算したモードを示し、画像の表示切替では曲線とピーク候補を変更しません。
別モードの曲線が必要な場合は、解析条件でそのモードを選び直してAnalyzeしてください。
...
比較表示の保存名とJSONの`mode`は表示中のモード、`tamura_mode`はCSVの曲線を計算したモードです。
比較に使った較正情報は、Gabor表示を保存する場合もJSONへ残します。
保存先と名前はダイアログで変更できます。
