...
ダウンロードしたアプリがmacOSのApp Translocationで一時的な場所から実行されても、元の `.app` の配置先を取得してその隣へ保存します。
アプリの隣に書き込めない場合はエラーを表示します。別の保存先へ自動変更しません。
元の配置先にも書き込み権限がない場合は、そのフォルダの権限を確認するか、`--output` で保存先を指定してください。
...
さらに利用許可済みのダウンロード属性をテスト用コピーに付け、macOS自身によるApp Translocation起動を再現します。
実行中のアプリが本物の一時マウント上にあることと、Rec OFFでの起動・模擬録画画像の保存先が元の `.app` の隣であることを検証します。
このテストではOS全体のセキュリティ設定を変更しません。
