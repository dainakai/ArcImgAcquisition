...
App Translocationで一時コピーから実行されても、元の `.app` の配置先を取得してその隣へ保存します。
元の配置先にも書き込み権限がない場合はエラーを表示します。そのフォルダの権限を確認するか、`--output` で保存先を指定してください。
...
録画画像は生データです。
光伝搬、幾何補正、GS位相回復をGUIで実行する場合は、同じリポジトリの [DualHolo Analyze](analyzer/README.md) を使います。
DualHolo Analyzeは今回のユーザー指定によりローカルCPUで計算し、結果を保存できる独立アプリです。
従来の研究用解析は、作業ルールに従ってリモートで実行します。
