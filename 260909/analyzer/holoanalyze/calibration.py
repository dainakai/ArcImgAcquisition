...
from .engine import Propagator, Reconstruction, intensity_input, tamura
...
class Calibration:
    ...
    def validate_for(self, pair, config):
        ...
        if self.metadata.get("version") != 1 or self.metadata.get("direction") != "cam0_output_to_cam1_source":
    ...
    def load(cls, path):
        ...
        with zipfile.ZipFile(path) as archive:
            if sum(f.file_size for f in archive.infolist()) > 200_000_000:
                raise ValueError("Calibration archive exceeds the 4096 × 4096 map limit")
        with np.load(path, allow_pickle=False) as d:
            maps = [d[n].copy() for n in ("map_x", "map_y", "valid_mask", "calibrated_mask")]
            metadata = json.loads(str(d["metadata"].item()))
        shape = maps[0].shape
        if len(shape) != 2 or not all(0 < n <= 4096 for n in shape) or any(a.shape != shape for a in maps):
        ...
        if metadata.get("version") != 1 or not metadata.get("quality_passed"):
...
def fit_maps(images, config, cancel, progress):
    ...
    xy, disp, conf = np.array(xy), np.array(disp), np.array(conf)
...
def build_calibration(pair, config, focus_depths, scan, cancel, progress):
    ...
    for camera in range(2):
        reconstruction = Reconstruction(np.sqrt(intensity_input(pair.frames[camera].image)), config, cancel)
        if focus_depths is None:
            best_score, best, index = -1, None, -1
            for i, z in enumerate(scan):
                rendered = reconstruction.render(float(z), cancel)
                score = tamura(rendered.filtered)
                if score > best_score:
                    best_score, best, index = score, rendered, i
                progress(f"Glass focus cam{camera}", i+1, len(scan), None)
            if index in (0, len(scan)-1):
                raise ValueError(f"Glass focus cam{camera} peaks at the scan boundary. Adjust the range or enter verified focus depths.")
        else:
            best = reconstruction.render(focus_depths[camera], cancel)
        focused.append(best.filtered)
        distances.append(best.z_mm)
        del reconstruction
    mx, my, valid, calibrated, stats = fit_maps(focused, config, cancel, progress)
    metadata = dict(version=1, direction="cam0_output_to_cam1_source", quality_passed=True,
                    ...
                    focus_depths_mm=distances, input_sha256=[fingerprint(f.image) for f in pair.frames],
                    interpolation="Lanczos4", additional_flip=False, **stats)
    cancel.check()
    return Calibration(mx, my, valid, calibrated, metadata)
