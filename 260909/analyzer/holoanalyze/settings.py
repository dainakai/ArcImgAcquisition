...
class SettingsDialog(QDialog):
    def __init__(self, config, parent=None):
        ...
        self.add_int(form, "パディング辺長", "padding_size", 16, 8192, "全光伝搬の正方形配列の辺長です。既定4096。入力場を中央に置き、周囲は場の平均値で埋めます。入力画像より小さい値では計算を拒否します。")
        ...
        self.add_float(form, "ピークの最小突出率", "peak_prominence_fraction", 0, 1, "", "ピークの突出量を、そのピーク値に対する比率で指定します。既定0.03。探索端は常に除外し、両側に下りがある山のみ候補にします。")
        self.add_float(form, "ピークの最小幅", "peak_min_width_samples", 1, 1000, " 点", "半突出量で測ったピーク幅の下限です。深度のサンプル数を単位に指定します。")
        self.add_int(form, "メモリキャッシュ", "cache_megabytes", 1, 4096, "再生系列ごとのメモリ上限（MiB）です。超過した深度も削除せず、一時ファイルへ保持します。")
        self.fields['cache_megabytes'].setSuffix(" MiB")
        directory = QLineEdit(config.cache_directory or "")
        tip(directory, "全深度を保持する一時キャッシュの保存先です。空欄はOSの一時フォルダ。SSDを推奨します。必要容量が不足する場合は探索開始前に通知します。")
        self.fields['cache_directory'] = directory
        form.addRow("一時キャッシュ先", directory)
        tabs.addTab(focus, "焦点・キャッシュ")
        calibration = QWidget()
